Param(
    $SiteName = "telemetry-test",
    # $SitePath = "$SiteRoot\$SiteName",
    $PackageName = "Alloy.Sample.Telemetry.UI",
...
$SiteRoot = "e:\TelemetryUI"
$IISSiteName = "Default Web Site\TelemetryUI"
...
function Transform-Config {
    ...
    Update-EPiXmlFile -TargetFilePath "$SitePath\wwwroot\Web.config" -ModificationFilePath "$TmpPackageFolder\Setup\Alloy.Sample.Telemetry.UI.web.config.transform" -Replaces "{basePath}=$SitePath\appData;{DbServer}=$DbServer;{DbDatabase}=$SiteName;{DbUserName}=$DbSiteUser;{DbPassword}=$DbSitePassword;"
}
